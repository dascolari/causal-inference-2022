read_csv(here(file.path("data", "nlsy97_clean.csv"))) %>%
  ...
  summarise(mean_days_incarcerated = (mean(months_incarcerated)*(365/12)), n = n(), .groups = 'drop') %>% 
  ...
  labs(x = "Race", y = "Male/Female Ratio", title = "Ratio of Male/Female per Person Incarceration Time")
